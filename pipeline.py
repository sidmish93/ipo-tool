...
class Pipeline:
    ...
    def _bse_lookup(self, bpage, name):
        ...
        for q in queries:
            ...
            best = None
            for r in data:
                if _norm(r.get("scripName", "")) == target:
                    best = r
                    break
            if best is None:
                for r in data:
                    n = _norm(r.get("scripName", ""))
            ...
            if best is None:
                best = data[0]
            return {
                "scripcode": str(best.get("strSricpCode", "")).strip(),
                "ticker": (best.get("shortName") or "").strip(),
                "isin": (best.get("Isin") or "").strip(),
                "bse_name": (best.get("scripName") or "").strip(),
            }
        return None
